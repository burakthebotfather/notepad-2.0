...
async def send_card_to_user(bot, original_msg: Message):
    ...
    # Карточка водителя в следующем виде: 
    #  A.
    #  Богдановича, 123 +»
    card = (
        f"{shop}\n\n"
...
async def schedule_check(message_id: int):
    ...
    if not context["corrected"]:
        old, new = update_rating(msg.from_user.id, -0.5)

        try:
            await msg.reply("Отметка не отредактирована. Рейтинг понижен на 0.5!")
        except:
            pass
...
async def handle_message(message: Message):
    ...
    if TRIGGER in text:
        ...
        reply = await message.reply("Проверка прошла успешно. Отметка принята. Рейтинг повышен на 0.02.")
    ...
    reply = await message.reply(
        ...
        "Рейтинг может быть повышен, если исходная отметка будет оперативно отредактирована. "
